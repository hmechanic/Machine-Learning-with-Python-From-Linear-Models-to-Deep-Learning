...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    Returns:
        ...
        parameters.
    """
    h_loss = label * (np.dot(theta, feature_vector) + theta_0)

    return max(0, 1 - h_loss)
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    for i in range(len(feature_matrix)):
        h_loss_array.append(hinge_loss_single(feature_matrix[i], labels[i], theta, theta_0))
    h_loss_array = np.array(h_loss_array)
    mean_h_loss = np.mean(h_loss_array)
    return mean_h_loss
...
def average_perceptron(feature_matrix, labels, T):
    ...
    average_theta = np.zeros((feature_matrix.shape[1],))
    average_theta_0 = 0
    ...
    average_theta = average_theta/counter
    average_theta_0 = average_theta_0/counter
...
def pegasos_single_step_update(
    ...
    Returns:
        ...
        completed.
    """
    update_cri = label * (np.dot(theta, feature_vector) + theta_0)
...
def classifier_accuracy(
    ...
    train_class_acc = accuracy(predicted_train_labels, train_labels)
    predicted_val_labels = classify(val_feature_matrix, theta, theta_0)
    val_class_acc = accuracy(predicted_val_labels, val_labels)
    return (train_class_acc, val_class_acc)
...
def bag_of_words(texts,stopword=False, remove_stopword=False):
    """
