...
def tabular_q_learning(q_func, current_state_1, current_state_2, action_index,
    ...
    q_func[current_state_1, current_state_2, action_index,
           object_index] = (1-ALPHA)*q_value + ALPHA*(reward + GAMMA *  max_q_next_state)
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        # TODO Your code here
        current_room_id = dict_room_desc[current_room_desc]
        current_quest_id = dict_quest_desc[current_quest_desc]
        action_index, object_index = epsilon_greedy(current_room_id, current_quest_id, q_func, epsilon)
        ...
        if for_training:
            # update Q-function.
            # TODO Your code here
            next_room_id = dict_room_desc[next_room_desc]
            next_quest_id = dict_quest_desc[next_quest_desc]
            tabular_q_learning(q_func, current_room_id, current_quest_id, action_index,
                                   object_index, reward, next_room_id, next_quest_id,
                                   terminal)
...
if __name__ == '__main__':
    ...
    mean_plot_values = np.mean(epoch_rewards_test, axis=0)
    mean_reward = np.mean(mean_plot_values[50:])
    
    conv_indices = np.where(np.abs(mean_plot_values - mean_reward) < 0.01)[0]
    print("First convergence index:", conv_indices[0])
