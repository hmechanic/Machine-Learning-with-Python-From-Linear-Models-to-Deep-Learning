...
def project_onto_PC(X, pcs, n_components, feature_means=0):
    ...
    if feature_means == 0:
        centered_X, feature_means = center_data(X)
    else:
        centered_X = X - feature_means
    projection = centered_X @ pcs[:, :n_components] 
...
def reconstruct_PC(x_pca, pcs, n_components, X):
    ...
    # This is x_pca @ V.T
    reconstructed_centered = np.dot(x_pca, V.T)
    
    x_reconstructed = reconstructed_centered + feature_means
